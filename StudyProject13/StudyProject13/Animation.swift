...
extension Animation {
    ...
    static func paintYellow(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.backgroundColor = .systemYellow }
    }
    
    static func paintRed(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.backgroundColor = .systemRed }
    }
    
    static func translation(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.transform = .init(translationX: 100, y: -100) }
    }
    
    static func rotate(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.transform = .init(rotationAngle: CGFloat.pi) }
    }
    
    static func scaleUp(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.transform = .init(scaleX: 2, y: 2) }
    }
    
    static func identityTransform(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.transform =  .identity }
    }
    
    static func makeItRound(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.layer.cornerRadius = 75 }
    }
    
    static func makeItSquare(duration: TimeInterval) -> Animation {
        return Animation(duration: duration) { $0.layer.cornerRadius = 0 }
    }
}
...
// отличие в одну строчку, поэтому не стал использовать
extension UIView {
    func runningPropertyAnimator(_ animations: [Animation], options: UIView.AnimationOptions) {
        guard !animations.isEmpty else { return }
        
        var animations = animations
        let animation = animations.removeFirst()
        
        UIViewPropertyAnimator.runningPropertyAnimator(withDuration: animation.duration, delay: 0, options: options) {
            animation.handler(self)
        } completion: { _ in
            self.runningPropertyAnimator(animations, options: options)
        }
    }
}
